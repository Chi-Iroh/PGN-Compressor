...
bool can_bishop_move_to(struct coord from, struct coord to, enum player moving_player, struct board_state* state) {
    ...
    for (int distance = abs(file_diff); distance > 0; distance--) {
        ...
        if (distance != 1 && board_at_coord(state->board, from)->type != EMPTY_SQUARE) { // bishop cannot jump over pieces
            return false;
        }
    }
...
bool parse_bishop_move(struct move* move, const char* str, enum player moving_player, struct board_state* state) {
    ...
    ASSERT_PRINTF(find_starting_square(state, move, can_bishop_move_to), "Cannot find a starting square !\nMove: %s", move->algebraic_move);
    return move;
}
