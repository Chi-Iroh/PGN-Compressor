...
It contains every move, the result of the game, and may include extra information about, but not limited to, the place (city where the game happened) or the players' names.  
...
I assume a byte is worth 8 bits, because as far as I know, only ancient machines may not follow that convention, thus it's expected to be true for any modern computer.
It's important to keep this number in mind to compute the compression rate, as we'll focus more on individual bits than bytes.  

# Introduction

Note: PGN doesn't seem to be officially supported by Markdown syntax highlighting, so I used the R programming language highlighting.  
...
It's mandatory to know this notation to understand the project.  
...
Below tags are the moves. They use the [algebraic notation](https://en.wikipedia.org/wiki/Algebraic_notation_(chess)), introduced by the turn counter.  
At the end, there is the result of the game, either a draw or a win/loss.  

# Vocabulary

A <ins>piece</ins> refers to either a rook, a knight, a bishop, a king or a queen (so everything except pawns), and <ins>material</ins> means <ins>any piece or pawn</ins>.  

The chess board has 8 <ins>files</ins> and 8 <ins>ranks</ins>.  
<ins>Files</ins> are the columns, and they go from A (left) to H (right).  
<ins>Ranks</ins> are the lines, and they go from 1 (white side) to 8 (black side).  
...
However, according to this [PGN "specification"](http://www.saremba.de/chessgml/standards/pgn/pgn-complete.htm#c8.2.1) (quotes because I don't know if it's official and/or up-to-date), these turn numbers are optional.  
It means `e4 e6` seems to be a valid PGN, and both chess.com and lichess handle it correctly, so the compressor will not require them.  
Still, both chess.com and lichess seem to ignore turn indications, because when submitting nonsense PGN (i.e. `2. e4 e6 7. a3 a6`, turn numbers are obviously wrong, they should be 1 and 2 instead), they do not complain and treat it as if turn numbers didn't exist (here, same as `e4 e6 a3 a7`).  
The program behavior when encountering bad turn numbers is still to be determined, but it'll probably ignore turn numbers ar all while emitting a warning if they're erroneous.  

Another point from this paper is about periods. In practice seem to always end with a period (like `1.`), but having multiple periods or even not all appears to work on both chess.com and lichess.  
The compressor will generate correct turn numbers with a unique period, but will be able to input faulty PGN notation (bad turn numbers and/or inconsistent periods).  

Here's an insight of what syntaxes will be valid <ins>in the compressor</ins> :
<table>
    <tr>
        <th>Move</th>
        <th>Comment</th>
...
But sometimes, things are not that easy, let's take an example of how complex a move can be :  
...
Or the longest kind of moves, with 7 characters, `axb1=Q+` :  
- `a` the pawn on the A file
- `x` captures
- `b1` whatever was on B1
- `=Q` then promotes to a queen
- `+` and checks the opponent's king

## Ambiguous moves

As B8 square is located both on file B and the 8th rank, any rook on this file or this rank can move to B8, if there isn't material blocking the way.  
...
Thus the first `b` in the algebraic notation of the above move (`Rbxb8#`).  
...
Note that if there are multiple rooks in any direction, only one can move to the square, as it blocks others. As an example, let a rook on A1 and another one on A2. Only the one on A2 can go to A8, as the one on A1 is blocked by the one on B2.  
For a bishop, it's also up to 4, but for a knight and a queen, there are at most 8 squares (8 of those pieces able to move the same square at a given position).  
...
The obvious first step will be discarding them.  
...
It appears some pieces of information aren't strictly necessary and can be retrieved simply by looking at the board.  
For instance, when a move gives a check, it's not compressed.  
Instead, it's up to the decompressor to evaluate that move and then determine if it puts the opponent's king in check.  
If so, it's a check, and the decompressor outputs the relevant notation (here, the `+` at the end of the move).  

Another example is the `x` in the algebraic notation, used for captures. Here it won't be compressed either and it's up to the decompressor to add it back too.  

## Tags

Tags will be at the beginning of the output (compressed) file, for instance
...
Each tag is compressed as two C strings (each ends with a NUL byte).

A trailing NUL byte is added after the last one, so that the decompressor can determine the end of tags. If no tag is provided, a NUL byte is emitted anyway.  

## Moves

Let's go back to the moves.  
We established that something like `Rbxb8#` will be the one of the most complex kinds of moves we can encounter.  
...
The chess board is 8*8-sized, which is very convenient, as a square can fit in 6 bits (6 bits = 2<sup>6</sup> = 64 values).  
As an example, the move `Rd2` is compressed as `\100\011\001`, thus 9 bits instead of 24 (`Rd2` is 3 characters, 8 bits each).  

As we said earlier, check(mate) marks (+ and #) aren't compressed, thus only remains how to deal with ambiguous moves.  

(*) These values are followed by 1 or 2 extra bits :  
...
Usually, special moves (castling, promotion, alternative moves, etc) and annotations (comments, NAG, etc) are less frequent in PGNs that regular moves (a5, Kf5, Qe3, etc).  
Thus more frequent moves should be encoded more efficiently that rarer moves.  
We saw that there are 6 types of material (pawn + 5 pieces), leaving 2 spare possibilities (encoded in 3 bits = 2<sup>3</sup> possibilities = 8).  
The other 2 aren't enough for all special things, so they're used to introduce groups of those special moves/annotations.  
We saw that `\110` introduces castling and promotion, whereas `\111` is for comments, alternative moves, NAGs and end of the game.  
As castling and promotion are more frequent than the other ones, they only need a 4<sup>th</sup> bit to be encoded (`\110\0` for castling and `\110\1` for promotion), but the second group requires 5 bits (for instance, `\111\1\0` for NAGs).  

The same concept was used in [Discontinued ideas](#discontinued-ideas) to encode move numbers in the least amount of bits, given that only few games reach 100 moves, even if reaching thousands of legal moves is possible.  
...
It uses a dollar sign followed by a number between 0 and 255, after a move. For instance, `36. Re4 $23` means white's 36th move (rook to e4) puts black in zugzwang.  
They don't seem to be used very often (see [here](https://chess.stackexchange.com/questions/27834/does-anything-actually-use-numeric-annotation-glyphs-nags)), and about half of their possible values are not defined (see Wikipedia link), thus each site implements them as they want.  
As there are 256 values, they always will be encoded in a single byte, thus `$23` will become `\110\0\1\00010111` :
- `\111\1\0` : 5 introduction bits (see the beginning of this section)
...
After that come 2 bits to represent the piece in which the pawn promotes to :  
...
As an example, `a8=Q` (there's only one pawn on the 7th rank) is encoded as `\110\1\01` :

- `\110\1` = promotion
- `\00` = queen
- no index, as there's only one pawn

The same move `b8=Q` is then encoded `\110\1\00\1` if there's a pawn on B7, as this one can also promote.  
...
The other case is when the promoted pawn is on the A or H file :  
<img src="doc_assets/promotion_corner.png">  
Here the pawn still has two choices, push or capture, then two possible moves.  
If the decompressor detects a situation like that (multiple destination squares to be promoted), 1 or 2 extra bits will be added to store their zero-based file index (0 = smallest = closest to A, 1-2 = more to the right).  

As an instance, here's `axb1=Q` compressed :
- `\110\1\00` : promotion to queen
- `\1` : As we have 2 choices (A1 and B1), chooses the 2nd (because the bit is `1`) which is B1 (B > A)

On the other hand, `a1=Q` is encoded as :  
- `\110\1\00` : promotion to queen
- `\0` : A < B, first choice

Last, here's `bxc1=Q` from the previous example (with 3 ways to promote, in A1, B1 and C1) :
- `\110\1\00` : promotion to queen
...
As an instance, `axb3 e.p.` means a pawn in A4 captured an enemy pawn (which just moved to B4) and then landed on `B3`.  

En passant is a special pawn capture, thus a pawn moven and is encoded as a pawn move.  
However it has 2 different notations :  
- `exd4`, written like regular captures
- `exd4 e.p.`, with a suffix to explicitly show it was an en passant capture

Both notations are valid and can be used, they may even be mixed in the same PGN file.  
To avoid losing this information (that the `e.p.` suffix was used in the PGN file), the compressed file will contain the notation used by each en passant.  
...
The number of en passant will be stored at the very beginning of the compressed file, encoded using 4 bits, as 0 to 8 en passant may happen, thus 9 possibilities.  
...
If the game ends with a checkmate, then it's trivial to determine which player won the game, so nothing is encoded.  
However, players can resign early or agree on a draw, so in these cases, it must be encoded after the moves.  
...
And in the destination square is on an edge or in a corner, it may reduce this amount.  
...
<table>
    ...
    <tr>
        <th>To an edge but not in a corner</th>
    ...
    <tr>
        <th>Other squares (they have 8 squares around them)</th>
...
Thanks to those figures, we can optimize to the nearest bit, because the decompressor will be able to determine how many possibilities there are at most, and thus read the lowest amount of bits necessary.  
...
For instance, when the moved piece is a queen and the destination square is in a corner, there are up to 3 possibilities (depending on the presence of other material that may block the way). If there are indeed 3 possibilities then it's encoded in 2 bits.  

To encode the starting square, we'll take the index of its coordinate.  
As an example, let's consider a rook move, to A1.  
A1 being a corner, there are at most two rooks, one in the 1st rank and the other in the A file. We'll take A4 and D1 as examples.  

Coordinates will be converted into a zero-based number (from 0 to 63) :  

- A4 = 1st rank and 4th file = (1 - 1 = 0) * 8 + 3 = 3
- D1 = 4th rank and 1st file = (4 - 1 = 3) * 8 + 0 = 24
...
On our example with A4 and D1, it means that if the bit `\0` is read, the rook going to A1 is the one from A4, otherwise (bit `\1` was read) it's the one from D1.  
...
- `\000` = A file
- `\000` = rank 1
- `\0` = lowest ranked rook is moved (here, the one in A4 rather than D1)

Pawn moves cannot be ambiguous, as pawn move forward in straight lines.  
However, pawn captures can be ambiguous, and they're disambiguated exactly as piece moves (zero-based index computed from the pawn coordinates).  

Here are the three kinds of ambiguous pawn captures :  
<table>
    <tr>
...
- regular captures and en passant have different destination squares (regular = on the captured pawn, en passant = behind the captured pawn)

## Comments

A comment may contain relevant information about the game or the move its next to, as the name of the opening if placed in the first moves.  
...
- `\111` is either for comments, alternative moves, NAGs or end of the game
- `\0` is either for comments or alternative moves
- `\0` is only for comments
Then, the text is copied as it is in the PGN, and a NUL character is appended to end the comment.  
...
This example is from the [Swedish Wikipedia about PGN](https://sv.wikipedia.org/wiki/Portable_Game_Notation) (there is no alternative moves examples in the English page).  
Here, the parentheses contain a better move than what Black played.  
```r
1. e4 e5 2. Qh5 Nc6 3. Bc4 Nf6 (3... g6) 4. Qxf7# 1-0
```

Alternative moves are introduced by 5 bits `\111\0\1` :
- `\111` is for either comments, alternative moves, NAGs or end of the game
- `\0` introduces either comments or alternative moves
...
At the very beginning of the compressed file (even before the en passant header), there are 8 bits encoding a version number.  
This number starts at 0 and will be increased if the binary protocol will have breaking changes, in a future where this project is stable and maintained with releases.  
The version number would allow the decompressor to identify the protocol version, permitting to correctly process compressed files using an older protocol to be backward-compatible.  

## Compression order
```mermaid
graph TD
    A[Version number] --> B[Tags];
    B --> C[En passant header];
    C --> D[Moves];
    D --> E["End of the game (only if not checkmate)"];
```

# Complete example

Here is an example of a PGN which uses every notation :  
```r
...
The above PGN has 328 bytes / characters (if there isn't any newline), thus 328 * 8 = 2624 bits.  
Then the above table shows the file can be compressed into 1213 bits (151.625 bytes).  
2624 / 1213 ~= 2.163
1213 / 2624 ~= 0.4623
...
Comments and tags are unpredictable text and aren't compressed, so the more comments and tags there are in the PGN file, the less efficient the compression will be.  
...
```
This PGN file now takes 209 bytes (209 * 8 = 1672 bits) instead of 328.  
The compressed size was 1213 bits, now let's discard bits used by compressed tags and comments :  
- 184 : `Date` tag
- 149 : `Avoids en passant` comment
- 253 : `Blunder, Qe5+ wins the bishop.` comment
- 117 : `En passant !!` comment
- 173 : `Black has the attack` comment

Thus, 1213 - 184 - 149 - 253 - 117 - 173 = 337 bits (42.125 bytes), without comments and tags.  
1672 / 337 ~= 4.96  
337 / 1672 ~= 0.20  
This compressed PGN takes about 5 times less space than the plain PGN file, so the compression rate is about 20%.  
It's clearly much better without uncompressed text, and the game moves are unaltered.  

# Discontinued ideas <a id="discontinued-ideas"></a>

Before I figured out comments/alternative moves/NAGs can be encoded at the same place they appear in the PGN, I thought about encoding them at the end of the file, right after the moves.  

In order to save the idea and especially the reasoning, I'll put it below, even if it won't be used, it although contains some interesting pieces of information.  

---

Comments are stored after the last move has been encoded, starting with 3 bits `\110`, as shown in table in above "moves" section.  
...
To set up a margin of error, the biggest value (9 000) must be represented. 14 bits are necessary to hold 9 000, they can in fact store 16 384 numbers.  
Take note that moves are 1-based (starting at 1), but when encoded it will be 0-based (starting from 0).  
Examples :  
Here the trailing NUL character is added to keep these examples straightforward, but don't forget that it appears only if there's a comment or an alternative move after it (i.e. the last comment doesn't end with a NUL byte, it's just the end of the file).  

As many games end way before the 100th move, it's suboptimal to always encode comment move numbers on 14 bits, as most of those bits will be 0 very often.  
To address that issue, before even reading the move number, there is a single form bit. This bit will allow encoding comment move numbers with less bits (6, allowing 2<sup>6</sup> = 64 moves), to use less space in the common use case.  
If that form bit is 0, then the short form (6 bits) is used, otherwise the long form (14 bits) is used.  
